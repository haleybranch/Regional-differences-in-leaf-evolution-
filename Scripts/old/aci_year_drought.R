...
aci_data <- aci_data %>% filter(Vcmax < 200) #outlier
aci_data <- aci_data %>% filter(!is.na(Vcmax))

#####
mod1 = lmer(Vcmax~ Rep + Treatment*Region*PrePeak + (1|Site/ID),data=aci_data)
...
dm <- summary(mod1)
Vcmax_aov <- anova(mod1) # Region:PrePeak 0.05, 3way = 0.05
...
#Results:
# Marginal significance (p=0.1) for DNPe-WNPe)
...
Res_v_all_plot <-ggplot(Vcmax_res_se,(aes(x=level_order,y=m,ymax=upper,ymin=lower)))+
  geom_point()+geom_errorbar(width=0.25)+
  facet_grid(.~Region)+
  geom_line(aes(group=PrePeak, linetype=PrePeak))+
  scale_y_continuous(name=expression(paste("Vcmax (", mu, molm^-2, s^-1, ")")))+
  scale_x_discrete(name= "")+
  theme_classic()+
  theme(axis.text.x=element_blank())+
  theme(legend.position = "none")
Res_v_all_plot <-Res_v_all_plot + theme(
  axis.text.y = element_text(size=12,face="bold"),
  axis.title.y = element_text(color="black", size=16,vjust = 2, face="bold",hjust=0.5))
Res_v_all_plot <- Res_v_all_plot + facet_wrap(.~Region) +
